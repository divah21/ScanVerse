...
export function FeaturesSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="py-24 bg-gradient-to-br from-slate-800 via-gray-900 to-slate-900 relative overflow-hidden"
    >
      {/* Background decorations */}
      <div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-full blur-3xl opacity-30 -translate-x-1/2 -translate-y-1/2" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-gradient-to-r from-blue-600/20 to-indigo-600/20 rounded-full blur-3xl opacity-30 translate-x-1/2 translate-y-1/2" />

      {/* Floating particles */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="feature-particle absolute top-20 left-[10%] w-2 h-2 bg-purple-400/40 rounded-full"></div>
        <div className="feature-particle absolute top-40 left-[20%] w-1 h-1 bg-blue-400/40 rounded-full"></div>
        <div className="feature-particle absolute top-60 left-[80%] w-1.5 h-1.5 bg-pink-400/40 rounded-full"></div>
        ...
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="group p-8 bg-gray-900/60 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 border border-gray-700/50 relative overflow-hidden hover:bg-gray-800/70"
              >
                ...
                <div className="relative">
                  ...
                  <h3 className="text-xl font-bold text-white mb-4 text-center group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-pink-400 transition-all duration-300">
                    {feature.title}
                  </h3>
                ...
                {/* Hover effect border */}
                <div className="absolute inset-0 rounded-3xl border-2 border-transparent group-hover:border-purple-500/20 transition-all duration-500" />
              </div>
            )
          })}
        </div>
